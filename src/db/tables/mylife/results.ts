...
export function dailyWeights(user_id: string) {
    ...
    WHERE
        ex.created_at >= SELECT UTC_TIMESTAMP()
            AND ex.created_at < SELECT UTC_TIMESTAMP() + INTERVAL 1 DAY
            AND ex.user_id = ?
    GROUP BY set_id
...
export function dailyCardio(user_id: string) {
    ...
    FROM
        mylife_cardios
    WHERE
        created_at >= SELECT UTC_TIMESTAMP()
            AND created_at < SELECT UTC_TIMESTAMP() + INTERVAL 1 DAY
            AND user_id = ?
    ORDER BY created_at;
    `,
		[user_id]
